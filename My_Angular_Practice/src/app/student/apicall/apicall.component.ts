...
export class ApicallComponent {
 ...
 submit(){
   ...
   let endpoint = 'admin';
   this.ApiCallService.postApiCall(endpoint,this.signUpForm.value).subscribe((response: any) =>{
 ...
 inputBox(){
  ...
  if(this.studentName.length > 2){
   this.ApiCallService.postApiCall(urlEndPoint,reqData).subscribe((resp: any) => {
     console.log(resp);  
    })
   }
